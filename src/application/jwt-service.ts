...
export const jwtService = {
    async createJWT(user:UserMongoIdT): Promise<AccessTokenT & RefreshTokenT> {
        const token = jwt.sign({user_id:user._id.toString()}, settings.JWT_SECRET, {expiresIn:settings.TIME_LIFE_ACCESS_TOKEN})
        const refreshToken = jwt.sign({user_id:user._id.toString()}, settings.REFRESH_TOKEN_SECRET, {expiresIn:settings.TIME_LIFE_REFRESH_TOKEN})
