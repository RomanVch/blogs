...
export class AuthService {
    ...
    async auth(auth:{loginOrEmail:string,password:string,userAgent:string,ip:string}): Promise<UserDevicesSessionsT | null> {
        ...
        const user = await this.usersDbRepository.getUserByLoginOrEmail(loginOrEmail)
        if (!user) { return null }
    ...
    async registrationConfirmation(code:string): Promise<MessageForResT<BodyForMessageT>>{
        const user = await this.usersDbRepository.getUserByConfirmedCode(code)
        ...
        const change =  await this.usersDbRepository.changeUserByIsConfirmedCode(user.id)
    ...
    async resendingRegistrationEmail (email:string): Promise<MessageForResT<BodyForMessageT>>{
        const user = await this.usersDbRepository.getUserEmail(email)
        ...
        const uuid = uuidv4()
        const changeCode = await this.usersDbRepository.changeUserByConfirmedCode(user._id.toString(), uuid)
    ...
    async passwordRecoveryEmail (email:string): Promise<MessageForResT<BodyForMessageT>>{
        ...
        const uuid = uuidv4()
        const changeCode = await this.usersDbRepository.changeUserPasswordCode(user._id.toString(), uuid)
    ...
    async getTokens (deviceId:string): Promise<AccessTokenT & RefreshTokenT|null>{
        const user = await this.usersDbRepository.getUserByDeviceId(deviceId)
        if(!user){ return null}
    ...
    async changePassword (newPassword:string, code:string){
        const user = await this.usersDbRepository.getUserByChangePasswordCode(code)
        ...
        const checkChangePassword = await this.usersDbRepository.changeUserPassword(user.id,passwordHash,passwordSalt)
